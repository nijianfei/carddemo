...
public class SyncDeviceStateTask {
    ...
    public void update() {
        ...
        if (syncLogShowRateTtatic-- <= 0) {
            ...
            log.info("\r\n\r\n任务状态:{}   设备数量:{}   设备号:{}    socket连接数：{}  threadMap.size：{}\r\n 【待处理队列】剩余数:{}  UserIdS:{}\r\n 【处理队列】剩余数:{} UserIdS:{}\r\n",
                    DeviceManage.isWord(), deviceNos.size(), getDeviceState(deviceNos), WebSocket.webSocketMap.size(),WebSocket.threadMap.size(), DeviceManage.taskQueueWait.size(), userId, DeviceManage.taskQueueCurrent.size(), userIdCurrent);
